/**
 * This example integrates all planets of the Solar System + the Sun
...
void heartbeat(struct reb_simulation* r){
    if (reb_output_check(r, 10000.)){
        ...
        // If this is the first output, create file and write header
        static int header_written = 0;                 // *** ADDED FOR CSV ***
        FILE* fs;

        if (!header_written){
            fs = fopen("state.csv", "w");              // *** CHANGED FOR CSV ***
            fprintf(fs, "t");                          // first column = time

            // write header for each body
            for (int i = 0; i < r->N; i++){
                fprintf(fs, ",x%d,y%d,z%d,vx%d,vy%d,vz%d", i, i, i, i, i, i);
            }
            fprintf(fs, "\n");
            fclose(fs);
            header_written = 1;
        ...
        // Append state row
        fs = fopen("state.csv", "a");                  // *** CHANGED FOR CSV ***
        fprintf(fs, "%e", r->t);                       // write time first
